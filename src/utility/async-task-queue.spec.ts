...
describe("AsyncTaskQueue", () => {
	...
	describe("FIFO 실행 순서", () => {
		...
		it("즉시 완료되는 작업들도 순서를 지키며 모두 실행된다", async () => {
			...
			expect(tracker.log.filter((entry) => entry.endsWith(":start"))).toEqual(["i1:start", "i2:start", "i3:start", "i4:start"])
	...
	describe("실패한 작업 처리", () => {
		...
		it("대기열을 거쳐 실행된 작업이 실패해도 그 뒤 작업이 실행된다", async () => {
			...
			expect(tracker.log.filter((entry) => entry.endsWith(":start"))).toEqual(["ok:start", "failed:start", "last:start"])
		})
