...
describe("SequentialTaskQueue", () => {
	 ...
	 */
	function createTask(name: string, delay: number, executionOrder: string[] = []) {
		return vi.fn(
			() =>
				new Promise((resolve) => {
	...
	it("작업은 완료 시간과 관계없이 FIFO 순서로 실행되어야 한다", async () => {
		...
		expect(executionOrder).toEqual(["task1:start", "task1:end", "task2:start", "task2:end", "task3:start", "task3:end"])
	})
	...
	it("작업이 실패해도 큐가 멈추지 않고 다음 작업을 실행해야 한다", async () => {
		...
		expect(executionOrder).toEqual(["failedTask:start", "failedTask:error", "nextTask:start", "nextTask:end"])
	})
